...
- [ ] Add MLflow tracing and GenAI evaluation. Combine deterministic numerical/citation checks with retrieval metrics such as hit-rate@k and MRR, plus precision@k and recall@k when relevance labels are sufficiently exhaustive, plus MLflow judges for retrieval relevance/groundedness/sufficiency, response relevance, correctness, safety, and project guidelines.
  ...
  - [ ] Add retrieval relevance/sufficiency evaluation using the now-correct retrieval-span semantics, and keep component-level hit-rate/MRR holdout metrics separate from end-to-end report judges. Route-aware evaluation now scores only the filtered evidence actually supplied on each `(symbol, topic)` RETRIEVER route, avoiding stock cross-route aggregation. Focused offline gate: 42 MLflow-evaluation tests passed with Ruff clean. Live E1 run `94a450e54a0d47818a7ce93ca21002f2` returned perfect route relevance/sufficiency with zero empty routes. Initial live E2 run `58b33f05f8f943aa871b9ac4038b97c9` exposed comparison-query contamination (`retrieval_relevance/mean = 0.588`, AAPL recent = 0.25, MSFT recent = 0.0). After isolating each comparison retrieval query by company, live E2 run `482216ed2040430bb25a740f88e5270a` improved overall relevance to 0.933, AAPL recent to 0.8, AAPL risk to 1.0, and MSFT risk to 1.0, while MSFT recent remained empty/insufficient and overall sufficiency remained 0.75. The remaining measured weakness is bounded to comparison-mode MSFT recent-development recall after filtering.
