...
**Next:** Restore live research after the Databricks App resource-update regression, attach the optional Silver `daily_prices` resource, and live-check single-company and comparison charts before enriching evidence metadata and enabling session-bound follow-up chat.
...
- [ ] Build configured stock/period selection, charts, comparison metrics, a cited report, and evidence-grounded follow-up chat.
  ...
  - [ ] Render Overview/Market/Fundamentals from controlled data with exact as-of/status semantics and comparison-safe charts/tables.
    ...
    - [ ] Offline-verify the chart adapter/runtime/presenter/shell tests and bundle validation, then deploy and live-check AAPL and AAPL/MSFT charts. On 2026-09-07 the focused gates passed (4 history-adapter tests, 5 presenter tests, 26 app tests, 8 Databricks-App-runtime tests, Ruff, diff check, and bundle validation). Bundle planning correctly reported one app update, but `bundle deploy` was blocked by the Apps API update mask including unsupported `forward_user_access_token`; source files still uploaded and `bundle run` started successfully. Because the new optional `daily_prices` binding was therefore absent, the app initially fell back to preview. The live-resource gate was corrected so only the existing warehouse/Gold/index resources determine live research; missing `daily_prices` now suppresses only the chart.
