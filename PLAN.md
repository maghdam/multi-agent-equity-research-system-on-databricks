...
**Current position:** Milestone 1. All four Bronze MVP ingestion pipelines, all four Silver MVP transformations, and both Gold analytical metric tables are implemented and live-verified in Databricks. Credential-free GitHub Actions CI runs Ruff correctness lint and all 195 offline contract/unit tests on pull requests and pushes to `main`. The daily Alpaca market/news path is now bundle-orchestrated and live: prices and news ingest with overlapping incremental windows, Silver snapshots rebuild deterministically, Gold market metrics refresh, and a final verification task checks scope, uniqueness, coverage, freshness, and Gold-to-Silver provenance. Successful incremental runs, failure containment with no partial Bronze publication, overlapping safe reruns, and the final automated verification gate have all been live-verified. The daily schedule is enabled for 01:00 America/New_York Tuesday-Saturday. Remaining Milestone 1 work includes the lower-frequency SEC/fundamentals scheduled refresh and verification, durable operational audit/rejection reporting where required, and controlled deployment verification.

**Next:** Add the lower-frequency SEC/fundamentals refresh orchestration and verification gate; then complete controlled deployment verification.
...
- [ ] Configure initial backfills and scheduled incremental refreshes; verify quality, freshness, missing-data reporting, failure visibility, and safe reruns.
  ...
  - [x] Daily market/news refresh: enable the schedule at `01:00 America/New_York` Tuesday-Saturday after live verification.
  - [ ] Add the lower-frequency SEC company-facts/filings -> Silver -> Gold fundamentals refresh and its operational verification gate.
